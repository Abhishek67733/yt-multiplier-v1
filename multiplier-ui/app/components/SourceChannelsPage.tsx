...
import {
  ...
  Eye, ThumbsUp, Clock, Flame, ExternalLink, MessageCircle, Hash,
  Calendar, TrendingUp, Search, ArrowUpDown, Copy,
} from "lucide-react";
...
function ShortsTable({
  ...
  const SortHeader = ({ label, k, icon: Icon }: { label: string; k: SortKey; icon?: any }) => (
    <th
      onClick={() => onSort(k)}
      className="text-left px-4 py-2.5 font-medium cursor-pointer hover:text-[#888] transition-colors select-none"
    >
      ...
      <table className="w-full">
        <thead>
          <tr className="text-[10px] uppercase tracking-wider text-[#444] border-b border-[#1C1C1C]">
            ...
            <th className="text-right px-4 py-2.5 font-medium">Dur.</th>
        ...
        <tbody>
          {sorted.map((s, idx) => (
            <tr key={s.video_id} className="border-b border-[#141414] hover:bg-white/[0.02] transition-colors group">
              <td className="px-4 py-3 text-[11px] text-[#444]">{idx + 1}</td>
              <td className="px-4 py-3">
                <div className="flex items-center gap-3 max-w-[220px]">
                  <img src={thumbUrl(s.thumbnail, s.video_id)} alt="" className="w-14 h-9 rounded-lg object-cover flex-shrink-0 ring-1 ring-white/5" />
              ...
              <td className="px-4 py-3 text-right">
                <span className={`text-[13px] font-semibold tabular-nums ${
                  s.views_delta >= 1000 ? "text-red-400" : s.views_delta >= 500 ? "text-orange-400" : "text-[#555]"
                }`}>
                  +{fmt(s.views_delta)}
                </span>
              </td>
...
function ChannelRow({ ch, shorts, onRemove }: { ch: Channel; shorts: Short[]; onRemove: () => void }) {
  ...
  return (
    <div className="bg-[#111] border border-[#1C1C1C] rounded-2xl overflow-hidden hover:border-[#2A2A2A] transition-colors">
      <div
        className="flex items-center gap-4 px-5 py-4 cursor-pointer group"
        onClick={() => setExpanded(!expanded)}
      >
        {ch.thumbnail ? (
          <img src={ch.thumbnail} alt="" className="w-11 h-11 rounded-full object-cover ring-2 ring-[#2A2A2A] flex-shrink-0" />
        ) : (
          <div className="w-11 h-11 rounded-full bg-[#1C1C1C] flex items-center justify-center flex-shrink-0">
            <PlaySquare className="w-5 h-5 text-[#444]" />
          </div>
        )}

        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-white font-medium text-sm truncate">{ch.name || ch.url}</p>
            {viral > 0 && (
              <span className="flex items-center gap-1 bg-red-500/15 text-red-400 text-[10px] font-bold px-2 py-0.5 rounded-full border border-red-500/25 animate-pulse-red">
                <Flame className="w-3 h-3" /> {viral} viral
              </span>
            )}
          </div>
          <p className="text-[11px] text-[#555] mt-0.5 truncate">{ch.url}</p>
        </div>

        <div className="flex items-center gap-5 text-xs text-[#555] flex-shrink-0">
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <div>
            <h2 className="text-white font-semibold text-base">Source Channels</h2>
            <p className="text-[11px] text-[#555] mt-0.5">
              {channels.length} channels · {shorts.length} Shorts tracked
              {totalViral > 0 && (
                <span className="text-red-400 font-medium ml-1">· {totalViral} viral</span>
              )}
            </p>
          </div>
        </div>
      ...
      {channels.length > 0 && (
        <div className="grid grid-cols-4 gap-3 mb-6">
          {[
            { label: "Channels", value: channels.length, color: "text-white" },
            { label: "Shorts Tracked", value: shorts.length, color: "text-white" },
            { label: "Total Views", value: fmt(totalViews), color: "text-sky-400" },
            { label: "24h Delta Total", value: `+${fmt(totalDelta)}`, color: "text-red-400" },
          ].map((s) => (
            <div key={s.label} className="bg-[#111] border border-[#1C1C1C] rounded-xl px-4 py-3">
              <p className="text-[10px] uppercase tracking-wide text-[#444] mb-1">{s.label}</p>
